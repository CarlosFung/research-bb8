...
def prepare_mask(logit_mask, index):
    """
    :param logit_mask: The mask images with [N, h, w, 1]. Values should be scaled [0,1]
...
# It expects to find the data the same shape with [N, h, w, channels]
def prepare_data_RGB_6DoF(filename):
    ...
    Xtr = data["Xtr"]  # RGB image data [N, h, w, channels]

    Ytr = data["Xtr_mask"]  # the mask [N, h, w, 1]
